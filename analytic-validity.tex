...
%\todo{This may need to be explained for non-economists - why is lagged employment of firm e a function of industry effect alpha i?}
...
\begin{center}
    Table~\ref{tab:m2sargan} about here.
\end{center}

While the coefficients are broadly biased with respect to coefficients obtained from the confidential data, specification tests such as  the $m2$ test for autocorrelation and the Sargan test paint a slightly less consistent picture. Table~\ref{tab:m2sargan} shows the two tests for each of the models estimated by country, synthetic status, and model. 

broadly yield the same conclusion.  For example, the Sargan test rejects the null hypothesis that the overidentifying restrictions are valid, symmetrically in both synthetic and confidential data. Thus, a user basing an estimation strategy on the choice of models in the synthetic data would have chosen the the same models as with the confidential data.
